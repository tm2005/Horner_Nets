"""Educational example: linear first-order ODE.

Problem:
    y'(t) + 2 y(t) = 1,    y(0) = 1

The exact solution is y(t) = 0.5 * (1 + exp(-2t)). This script shows the same
PINN/INR procedure as 1order_1.py, but on a linear equation:
1. sample points in the domain,
2. compute y' with autograd,
3. minimize the ODE residual and soft initial condition,
4. compare the learned function with the exact solution.
"""
...
# ODE: y' + 2y = 1, y(0) = 1
# Exact solution: y(t) = 0.5 * (1 + exp(-2t))
def rhs(t):
