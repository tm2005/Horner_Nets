"""Horner-spline example for the nonlinear first-order ODE.
...
This script follows the same form as `Horner_1D/our_1order_1.py`, but the model
is a piecewise Horner spline. The first interval receives the hard initial
condition y(0)=1. Later intervals receive their left value from the previous
interval, so the spline is continuous by construction.
"""
...
from Model_Horner_Spline import HornerSplineIC1
from My_Fncs import (
    count_parameters,
    derivative,
...
# The spline has one Horner polynomial per interval.
BREAKPOINTS = [T_START, 1, 2, T_END]
HORNER_ORDERS = [7, 7, 7]
...
class Dataset1stSplineHardIC(Dataset):
...
def build_model():
    """Create the piecewise hard-IC Horner spline model."""

    return HornerSplineIC1(BREAKPOINTS, HORNER_ORDERS, initial_value=Y_INIT)
...
def residual_loss_1st_order(t, u, model, interval_idx, loss_eq):
    """Compute the ODE residual loss on one spline interval."""
    ...
    residual = ode_residual(y, yd, u_sub)
    return loss_eq(residual, torch.zeros_like(residual))


def train_1st_order_spline_lbfgs(dataloader, model, optimizer, loss_eq, device):
    """Train one outer LBFGS step for the first-order spline model."""
    ...
    for t, u, t_init, y_init in dataloader:
        ...
        def closure():
            ...
            losses = [
                residual_loss_1st_order(t, u, model, interval_idx, loss_eq)
...
if __name__ == "__main__":
    ...
    train_data = Dataset1stSplineHardIC(t, u, T_INIT, Y_INIT)
    train_dataloader = DataLoader(train_data, batch_size=None, shuffle=False)
    train_error = [0.0] * EPOCHS

    print("Model: HornerSplineIC1")
    print("ODE: y * y' = t, y(0) = 1")
    print(f"Breakpoints: {BREAKPOINTS}")

    for epoch in range(EPOCHS):
        train_error[epoch], sub_losses = train_1st_order_spline_lbfgs(
